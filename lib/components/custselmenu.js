...
class Menu extends Component {
  ...
  setCtrStyles() { // return styles for the outer container based on props and state
    ...
    if (!this.props.showMenu) {
      ...
      styles.zIndex = this.props.openZIndex;
  ...
  focusOut(e) {
    let ctr = this.refs.theContainer;
    let nwtgt = e.relatedTarget;
    // if (!nwtgt) {
    //   return;
    // }
    while (nwtgt) {
  ...
  adjustWithDOM() {
    ...
    let top = this.props.btnBox.top - (liHeight - this.props.btnBox.height) / 2;
    ...
    if (top + height > this.props.vPort.height - this.props.bottomMargin) {
      ...
      let diff = maxHeight - height;
      if (diff > 0) {
        let ctop = Math.max(top - diff, this.props.topMargin);
        diff = top - ctop;
        height += diff;
        top = ctop;
        physProps.scrollAmt -= diff;
        if (physProps.scrollAmt < 0) {
          height += physProps.scrollAmt;
          top -= physProps.scrollAmt;
          physProps.scrollAmt = 0;
        }
      }
      physProps.height = height;
    }
    physProps.top = top - this.props.btnBox.top;
  ...
  renderList(list, level, parId) {
    ...
    return (
      ...
      >
        {list.map(item => {
          let tabIndex, className, id, role, clickFunc, focusoutFunc,
            itemClickFunc, refType = null;
          if (item.children) {
            className = this.props.groupClass;
            id = this.props.idPrefix + item.value;
            itemClickFunc = this.ignoreClick;
            tabIndex = '-1';
          } else {
            className = this.props.leafClass;
            tabIndex = '0';
            focusoutFunc = this.focusOut;
            role = 'option';
...
Menu.propTypes = {
  ...
  containerClass: PropTypes.string,         // CSS class for the container
  idPrefix: PropTypes.string,
  openZIndex: PropTypes.number
};
